...
mod tests {
    ...
    #[test]
    fn test_append_hint() {
        let result = ToolResult::text("original".into());
        let result = result.with_hint("\n[nudge]");
    ...
    fn test_append_hint_empty_content() {
        ...
        let result = result.with_hint("[hint]");
    ...
    #[test]
    fn test_jsonrpc_version_rejected() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("unsupported JSON-RPC version")
        );
    }
