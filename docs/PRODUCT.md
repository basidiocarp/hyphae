...
Hypae is a permanent memory system for AI agents. One binary, zero dependencies, MCP native.

### 5 Differentiators

**1. Two memory models**

Not everything deserves the same treatment. Episodic memories (decisions, errors) decay naturally. Semantic knowledge (architecture graphs) persists forever. Hypae models both.

**2. Zero LLM cost for extraction**
...
`hyphae init` configures 14 tools in one command. Works with Claude Code, Cursor, VS Code, Windsurf, Zed, Amp, Amazon Q, Cline, Roo Code, Kilo Code, Codex CLI, OpenCode, Claude Desktop, Gemini. Switch tools without losing memory.
...
Session 1 (Friday):
  ...
  → All stored in Hypae automatically
...
New developer's agent gets full architectural context from day one via `hyphae recall-context`.
...
Hypae is MCP-native. All tools connect to the same memory. Backend decisions made in Claude Code are recalled in Cursor. Error fixes from Codex sessions inform future Claude Code work.
...
Hypae's auto-decay ensures only important information persists:
...
Hypae's context injection works without tool use:
...
| Metric | Without Hypae | With Hypae | Delta |
|--------|---------------|------------|-------|
...
| Model | Params | Without Hypae | With Hypae | Delta |
|-------|--------|---------------|------------|-------|
...
No cloud. No API key for storage. No Docker. No configuration beyond `hyphae init`.
